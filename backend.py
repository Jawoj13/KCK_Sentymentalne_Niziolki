...
class EvaluationController:
    ...
    # NOWA METODA: Aktualizuje typ ćwiczenia w locie
    def set_exercise_type(self, exercise_type):
        if self.exercise_type != exercise_type:
            self.exercise_type = exercise_type
            self.segmenter.set_exercise_type(exercise_type)
            self.reset()
...
class SyncInferenceWorker(QThread):
    ...
    def __init__(self, queue_a, queue_b, exercise_type="full", target_repetitions=10, dominant_side="left",
        ...
        # Atrybut sterujący wyborem z interfejsu (main.py)
        self.exercise_type = exercise_type

        self.evaluation = EvaluationController(self.exercise_type, target_repetitions, dominant_side)
    ...
    def run(self):
        while self._is_running:
            # Synchronizacja typu ćwiczenia z głównym wątkiem (interfejsem)
            if hasattr(self, 'exercise_type') and self.exercise_type != self.evaluation.exercise_type:
                self.evaluation.set_exercise_type(self.exercise_type)
